...
How to deploy color in an interface: semantic tokens, meaning, emphasis, and appearance variants. For picking the values themselves, see [palette-generation.md](colors-palette-generation.md); for checking pairs, see [accessibility-contrast.md](colors-accessibility-contrast.md).
...
If a role has no token yet, add the token; don't borrow one that happens to have the right value today. In Tailwind projects, this is the `@theme` block; see [gamut-and-tailwind.md](colors-gamut-and-tailwind.md).
...
Every custom color needs a light and a dark variant; dark mode derivation is covered in [palette-generation.md](colors-palette-generation.md). Beyond that, users who enable increased contrast expect visibly stronger differentiation; supply it with `prefers-contrast`:
