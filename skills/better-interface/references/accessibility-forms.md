...
- Never use `user-scalable=no` or `maximum-scale=1` to stop iOS input zoom; keep mobile input text at `16px` instead (covered in the Typography domain).
...
- Announce results: success goes through a polite live region. For submit failures, focus the first invalid field; the focus move is the announcement, and reserve `role="alert"` for form-level errors not tied to a field (see [screen-readers.md](accessibility-screen-readers.md)).
...
- Preserve native Enter-to-submit behavior for form inputs. Add ⌘/Ctrl+Enter submission in a `<textarea>` only when it is an intentional product convention, implement it explicitly, and provide another visible submission control.
