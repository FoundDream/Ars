...
- [Enter animations](#enter-animations-split-and-stagger)
...
**Fallback recipe:** When the project has no established icon-transition values, start with:
- `scale`: `0.25` → `1`
- `opacity`: `0` → `1`
- `filter`: `"blur(4px)"` → `"blur(0px)"`
- `transition`: `{ type: "spring", duration: 0.3, bounce: 0 }`

Preserve nearby motion conventions when they are deliberate and accessible. These values express one restrained house style, not a universal requirement.

## Scale on Press

A subtle scale-down on click can give buttons tactile feedback. If the project has no press-motion token, start with `scale(0.96)` and avoid values below `0.95`, which usually feel exaggerated. Use an interruptible transition and honor reduced motion.
...
Honoring `prefers-reduced-motion` is covered by the Accessibility domain; apply it to every animation in this file.
