...
- Use `touch-action: manipulation` only when testing shows delayed activation or gesture conflict and disabling double-tap gestures is acceptable; never use it as blanket cleanup.
- Preserve visible tap feedback. Customize `-webkit-tap-highlight-color` only when the replacement remains clear and the project intentionally controls this non-standard behavior.
