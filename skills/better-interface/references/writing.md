...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) is covered by the Typography domain; error markup and announcements (`aria-invalid`, live regions) by the Accessibility domain; room for translated strings by the Layout domain.
