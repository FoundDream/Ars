...
- [Selection](#selection)
- [Forms and editable text](#forms-and-editable-text)
- [Decorative text](#decorative-text)
- [Sizes](#sizes)
...
When text appears low-contrast, use the Colors domain to measure the rendered foreground/background pair and the Accessibility domain to classify the applicable requirement. Changing the project's colors remains a design decision unless the user asks for remediation.

## Font smoothing

Treat font smoothing as an optional, non-standard, macOS-specific rendering choice. Preserve browser defaults unless the project already uses it or side-by-side testing demonstrates that it supports the intended type rendering. Tailwind's `antialiased` sets both vendor-prefixed properties:

```css
html { /* only after platform comparison */
