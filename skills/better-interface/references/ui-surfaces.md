...
### Fallback color recipe

- Prefer the project's existing image or separator token when it works across appearances.
- Without one, start with pure black at low opacity in light mode, `oklch(0 0 0 / 0.1)`.
- Without one, start with pure white at low opacity in dark mode, `oklch(1 0 0 / 0.1)`.
- Keep the fallback neutral; tinted outlines can pick up the surrounding surface color and look dirty.
...
Use the project's verified token when one exists. Otherwise `outline-black/10` and `outline-white/10` provide a neutral fallback.
