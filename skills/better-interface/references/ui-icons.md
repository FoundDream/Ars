...
- [Match icon stroke to text weight](#match-icon-stroke-to-text-weight)
- [One SVG, recolored per state](#one-svg-recolored-per-state)
- [Outline default, fill active](#outline-default-fill-active)
- [Design at render size](#design-at-render-size)
- [Icons in RTL](#icons-in-rtl)
...
The swap between variants is a contextual icon animation; use the exact cross-fade values in [animations.md](ui-animations.md).
...
Analyze composite icons part by part: a badge or slash overlay may keep its position even when the base glyph flips. Accessible names for icon-only buttons are covered by the Accessibility domain.
