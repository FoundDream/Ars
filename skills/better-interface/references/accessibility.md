...
Rendered-pair contrast measurement and color remediation are covered by the Colors domain; visual text sizing and iOS input zoom by the Typography domain; spatial RTL layout by the Layout domain.
...
| `maximum-scale=1` to stop iOS input zoom | 16px input font on mobile (see the Typography domain); never block zoom |
