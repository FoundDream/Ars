...
APCA (Accessible Perceptual Contrast Algorithm) can supplement perceptual design evaluation. Use it only when a deterministic APCA calculator is available and the relevant font size, weight, and polarity are known. Never estimate an Lc value. Use WCAG 2 ratios for formal WCAG 2 conformance claims.

Lc (Lightness Contrast) measures perceived contrast between foreground and background. The levels below are simplified triage guidance, not a substitute for APCA's font-size/weight lookup table:
